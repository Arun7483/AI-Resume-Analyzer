...
export class ResumeService {
  ...
  readonly analysis = signal<ResumeAnalysisResponse | null>(null); readonly status = signal<'idle'|'uploading'|'analyzing'|'complete'|'error'>('idle'); readonly uploadProgress = signal(0); readonly error = signal<string|null>(null); readonly history = signal<ResumeHistory[]>([]); readonly historyLoading = signal(false); readonly historyError = signal('');
  ...
  loadHistory(): void { this.historyLoading.set(true); this.historyError.set(''); this.http.get<ResumeHistory[]>(`${this.apiUrl}/history`).subscribe({ next: history => { this.history.set(history); this.historyLoading.set(false); }, error: response => { this.history.set([]); this.historyLoading.set(false); this.historyError.set(response.status === 404 ? 'Resume history is not available on the deployed backend yet.' : 'Could not load resume history. Please refresh.'); } }); }
